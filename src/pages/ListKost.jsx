...
import { Tag, Space, Button, Card, Typography, Input, List, Badge, Select } from "antd"; // Tambah Select
import {
  PlusOutlined,
  EditOutlined,
  DeleteOutlined,
      ...
      <div style={{ marginBottom: 24, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "16px" }}>
        ...
        <Space size="middle" wrap>
          ...
          <Select
            ...
            size="large"
            allowClear
            onChange={(value) => setFilterKost(value)}
          >
            <Option value="Bahagia">Kost Bahagia</Option>
            <Option value="Melati">Kost Melati</Option>
            <Option value="Menteng">Kost Menteng</Option>
          </Select>
          ...
          <Select
            ...
            size="large"
            allowClear
            onChange={(value) => setFilterLantai(value)}
          >
          ...
          <Select
            ...
            size="large"
            allowClear
            onChange={(value) => setFilterKamar(value)}
          >
          ...
          <List.Item>
            ...
            >
              <Card
                ...
                actions={[
                  isAdminOrOwner && <EditOutlined key="edit" onClick={() => console.log("Edit", item.key)} />,
              ...
              >
                <Space direction="vertical" style={{ width: "100%" }} onClick={() => handleDetail(item)}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                    <Tag color={getTipeColor(item.tipe)}>{item.tipe.toUpperCase()}</Tag>
                    <Text strong style={{ color: "#1890ff", fontSize: "16px" }}>
                      Rp {item.harga.toLocaleString("id-ID")}
                      <small style={{ fontSize: "12px", fontWeight: "normal" }}>/bln</small>
                    </Text>
                  </div>
                  <Title level={5} style={{ margin: "8px 0 4px" }}>{item.nama}</Title>
