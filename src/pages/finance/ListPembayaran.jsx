...
const columns = (showPreviewModal, showEditModal, showDeleteDialog) => [
    ...
    },
    {
        ...
        width: '12%',
        render: (value) => (
            <Tag color={value === 'transfer' ? 'blue' : 'green'}>
                {value?.toUpperCase() || '-'}
            </Tag>
        ),
    },
...
const ListPembayaran = memo(function ListPembayaran(props) {
    ...
    const showDeleteDialog = (param) => {
        NotifConfirmDialog({
            ...
            message: `Payment dengan Order ID "${param.order_id}" akan dihapus?`,
            onConfirm: () => handleDelete(param.payments_id, param.order_id),
            onCancel: () => props.setSelectedData(null),
        });
    };

    const handleDelete = async (payments_id, order_id) => {
        try {
            ...
            if (response.statusCode === 200) {
                NotifAlert({
                    icon: 'success',
                    title: 'Berhasil',
                    message: `Data Payment "${order_id}" berhasil dihapus.`,
                });
                doFilter();
            } else {
                NotifOk({
                    ...
                    <Col xs={24}>
                        <Row justify="space-between" align="middle" gutter={[8, 8]}>
                            <Col xs={24} sm={24} md={12} lg={12}>
                                <Input.Search
                                    placeholder="Search payment by Order ID or Type Payment..."
                                    value={searchValue}
                                    onChange={(e) => {
                    ...
                    <Col xs={24} sm={24} md={24} lg={24} xl={24} style={{ marginTop: '16px' }}>
                        <TableList
                            ...
                            onGetData={(fetchedData) => {
                                // Optional: handle data after fetch
